...
(`cmd/cshared`). Compile-time link against `libitb3.so` with an rpath
...
finalization (libitb3 zeroes key material internally).

The units are declared as `Itb3` and its children; every example below
renames the root with `package Itb renames Itb3;` and then spells the
calls `Itb.*`. Child packages resolve through the renaming, so the
version stays confined to that one line and the code beneath it is
unaffected by a future major.
...
The convenience driver builds `libitb3.so` plus all four GNAT projects
...
cd bindings/ada
alr exec -- gprbuild -P libitb3.gpr
...
`libitb3.so` on a system loader path or `LD_LIBRARY_PATH` set.
...
procedure Round_Trip is
   package Itb renames Itb3;   --  the only line that names the library version
...
the file access inside libitb3.
...
enforced by libitb3; the binding treats the JSON as an opaque
`String`.
