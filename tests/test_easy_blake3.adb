--  BLAKE3-focused Easy Mode (Itb.Encryptor) coverage.
--
--  Symmetric counterpart to bindings/rust/tests/test_easy_blake3.rs
--  applied to the high-level Itb.Encryptor surface. BLAKE3 ships at a
--  single width (-256) — there is no -512 BLAKE3 in the registry — so
--  this file iterates the single primitive across the same axes
--  test_easy_blake2{b,s} cover.
--
--  Persistence rides on Itb.Encryptor.Export_State /
--  Itb.Encryptor.Import_State (JSON blob, single round-trip). The
--  blob captures the strictly larger encryptor state (PRF keys for
--  every slot, MAC key, optional dedicated lockSeed material) so the
--  day-2 decrypt path exercises the full restore.
--
...
procedure Test_Easy_Blake3 is

   Hash_Name : constant String := "blake3";
   Width     : constant Integer := 256;

   --  BLAKE3 carries a 32-byte fixed PRF key.
...
begin
   ...
   Ada.Text_IO.Put_Line ("test_easy_blake3: PASS");
