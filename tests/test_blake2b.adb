--  BLAKE2b-focused Ada binding coverage.
--
--  Symmetric counterpart to test_areion.adb: the same coverage shape
--  (nonce-size sweep, single + triple roundtrip, single + triple
--  auth with tamper rejection, persistence sweep, plaintext-size
--  sweep) applied to blake2b256 / blake2b512.
--
...
procedure Test_Blake2b is

   ------------------------------------------------------------------
   --  Per-primitive table — both BLAKE2b widths. Width feeds into
   --  Seed.From_Components key validation in the persistence test
   --  below; Expected_Key_Len locks in the FFI-surfaced contract
   --  that BLAKE2b-256 carries a 32-byte fixed key and BLAKE2b-512
   --  carries a 64-byte fixed key.
   ------------------------------------------------------------------
...
begin
   ...
   Ada.Text_IO.Put_Line ("test_blake2b: PASS");
