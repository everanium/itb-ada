--  AES-CMAC-focused Easy Mode (Itb.Encryptor) coverage.
--
--  Symmetric counterpart to bindings/rust/tests/test_easy_aescmac.rs
--  applied to the high-level Itb.Encryptor surface. AES-CMAC ships
--  only at -128 (the AES block size).
--
...
procedure Test_Easy_Aescmac is
   ...
   --  AES-CMAC carries a 16-byte AES-128 fixed PRF key.
   ...
   --  wall clock). A non-CSPRNG generator is acceptable here because
   --  ITB encryption is keyed by libitb's own internal CSPRNG; the
   --  test plaintext / MAC key only need to be non-trivial, not
   --  unpredictable.
   ------------------------------------------------------------------
...
begin
   ...
   Ada.Text_IO.Put_Line ("test_easy_aescmac: PASS");
