--  SipHash-2-4-focused Easy Mode (Itb.Encryptor) coverage.
--
--  Symmetric counterpart to bindings/rust/tests/test_easy_siphash24.rs
--  applied to the high-level Itb.Encryptor surface. SipHash-2-4 ships
--  only at -128 and is the unique primitive with no fixed PRF key —
--  Has_PRF_Keys is False, Get_PRF_Key surfaces Itb_Error / Bad_Input.
--  The persistence path therefore exports / imports without
...
procedure Test_Easy_Siphash24 is

   Hash_Name : constant String := "siphash24";
...
begin
   ...
   --  no_prf_keys — SipHash is the lone primitive with Has_PRF_Keys =
   --  False; Get_PRF_Key rejects indexed access with Bad_Input.
   declare
      Enc : constant Itb.Encryptor.Encryptor :=
        Itb.Encryptor.Make (Hash_Name, 1024, "kmac256", 1);
   begin
      if Itb.Encryptor.Has_PRF_Keys (Enc) then
         raise Program_Error with "siphash24 should report Has_PRF_Keys=False";
      end if;
      declare
         K : constant Byte_Array := Itb.Encryptor.Get_PRF_Key (Enc, 0);
         pragma Unreferenced (K);
      begin
         raise Program_Error with "Get_PRF_Key on siphash24 must raise";
      end;
   exception
   ...
   --  Round-trip 5: Persistence sweep without PRF keys. SipHash's
   --  seed components alone reconstruct the keying material; the
   --  exported blob omits prf_keys, and Import on a fresh encryptor
   --  restores the seeds without consulting them.
   declare
            ...
            if KB mod Width = 0 then
               for N of Nonce_Sizes loop
                  ...
                  begin
                     ...
                     if Itb.Encryptor.Has_PRF_Keys (Src) then
                        raise Program_Error
                          with "siphash24 must report Has_PRF_Keys=False";
                     end if;
                     declare
   ...
   Ada.Text_IO.Put_Line ("test_easy_siphash24: PASS");
