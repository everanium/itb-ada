--  Areion-SoEM-focused Easy Mode (Itb.Encryptor) coverage.
--
--  Symmetric counterpart to bindings/rust/tests/test_easy_areion.rs
--  applied to the high-level Itb.Encryptor surface. Iterates over
--  both Areion-SoEM widths — areion256 (-256) and areion512 (-512).
--
--  Persistence rides on Itb.Encryptor.Export_State /
--  Itb.Encryptor.Import_State (JSON blob, single round-trip) instead
--  of the lower-level components + hash_key pair captured per seed
--  slot.
--
...
begin
         ...
         for H of Hashes loop
            declare
               ...
               Enc    : Itb.Encryptor.Encryptor :=
                 Itb.Encryptor.Make (H_Name, 1024, "kmac256", 1);
            begin
               ...
               begin
                  if Pt /= Plaintext then
                     raise Program_Error
                       with "Single Easy mismatch at " & H_Name & N'Image;
                  end if;
               end;
            end;
         end loop;
      ...
      for N of Nonce_Sizes loop
         for H of Hashes loop
            declare
               H_Name : constant String := To_String (H.Name);
               Enc    : Itb.Encryptor.Encryptor :=
                 Itb.Encryptor.Make (H_Name, 1024, "kmac256", 3);
            begin
               Itb.Encryptor.Set_Nonce_Bits (Enc, N);
               declare
                  Ct : constant Byte_Array :=
               ...
               begin
                  if Pt /= Plaintext then
                     raise Program_Error
                       with "Triple Easy mismatch at " & H_Name & N'Image;
                  end if;
               end;
            end;
         end loop;
         ...
         for M_Ptr of Mac_Names loop
            for H of Hashes loop
               declare
                  H_Name : constant String := To_String (H.Name);
                  Enc    : Itb.Encryptor.Encryptor :=
                    Itb.Encryptor.Make (H_Name, 1024, M_Ptr.all, 1);
               begin
                  Itb.Encryptor.Set_Nonce_Bits (Enc, N);
                  declare
                     Ct : constant Byte_Array :=
         ...
         for M_Ptr of Mac_Names loop
            for H of Hashes loop
               declare
                  H_Name : constant String := To_String (H.Name);
                  Enc    : Itb.Encryptor.Encryptor :=
                    Itb.Encryptor.Make (H_Name, 1024, M_Ptr.all, 3);
               begin
                  Itb.Encryptor.Set_Nonce_Bits (Enc, N);
                  declare
                     Ct : constant Byte_Array :=
                  ...
                  for N of Nonce_Sizes loop
                     declare
                        ...
                        Src    : Itb.Encryptor.Encryptor :=
                          Itb.Encryptor.Make (H_Name, KB, "kmac256", 1);
                     begin
                        ...
                        begin
                           if Src_Key'Length /= H.Expected_Key_Len then
                              raise Program_Error
         ...
         for Sz of Plaintext_Sizes loop
            ...
            begin
               ...
               begin
                  if Pt /= Plaintext then
                     raise Program_Error
                       with "Size sweep mismatch at " & H_Name
                            & Sz'Image & " nonce" & N'Image;
                  end if;
               end;
            end;
         end loop;
   ...
   Ada.Text_IO.Put_Line ("test_easy_areion: PASS");
