--  SipHash-2-4-focused Ada binding coverage.
--
--  W128 counterpart to test_aescmac.adb. SipHash ships at a single
--  width (-128) and has no internal fixed key — its keying material
--  is the per-call seed components — so Expected_Key_Len for
--  siphash24 is 0 and the persistence path stores empty bytes for
--  the hash key.
--
--  Mirrors bindings/rust/tests/test_siphash24.rs one-to-one. Each
--  Rust #[test] fn becomes a single nested block in the main
--  procedure; the per-test serial_lock is unnecessary because each
--  test_*.adb main procedure compiles into its own executable and
--  runs in its own process with a fresh libitb global state.
--  Set_Nonce_Bits is saved at the procedure entry and restored on
--  exit anyway, so an exception mid-test still leaves the global at
--  a valid value.
...
procedure Test_Siphash24 is

   Hash_Name : constant String := "siphash24";
   Width     : constant Integer := 128;

   --  SipHash-2-4 has no internal fixed key (keyed by seed
   --  components), so the FFI hash_key field is empty.
   Expected_Key_Len : constant Stream_Element_Offset := 0;
...
begin
   ...
   Ada.Text_IO.Put_Line ("test_siphash24: PASS");
