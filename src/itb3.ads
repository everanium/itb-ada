...
--  MAC / envelope / wire-format decision stays inside libitb3. The
--  wrapper surface is split across child packages:
--
--    Itb3.Status    Numeric status codes mirrored from the C ABI
--                  (cmd/cshared/internal/capi/errors.go).
--    Itb3.Error     Itb_Error exception + structured-payload accessors.
--    Itb3.Opts      URL-query builder for the opts pass-through string.
--    Itb3.Pipeline  Triple Pipeline session — Init / Load / Save /
...
--    Itb3.Runtime   Library version string + Go runtime knobs.
--
--  C ABI source-of-truth: dist/<os>-<arch>/libitb3.h. The raw FFI
...
package Itb3 is
   ...
   --  Opaque libitb3 object handle. Internal to the binding — exposed
...
private
   ...
   --  `extern int ITB_*` prototype in libitb3.h; C `size_t` maps to
