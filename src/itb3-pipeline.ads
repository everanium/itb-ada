...
--  entry points relay buffer-in / buffer-out to libitb3; the
--  controlled wrapper frees the Go-side handle at Finalize (libitb3
...
package Itb3.Pipeline is
   ...
   --  Itb_Error with Itb3.Status.Recipe_Primitive_Unknown; a record
   --  failing the profile field rules with
   --  Itb3.Status.Blob_Malformed_Recipe.
   procedure Load
     (P    : in out Pipeline;
      Blob : Byte_Array);

   --  Load variant overriding the blob-embedded parallax + wrapper
   --  masters (the pair is validated by libitb3).
   procedure Load
     (P           : in out Pipeline;
      Blob        : Byte_Array;
   ...
   --  Load for a blob stored at Path; the file is read inside libitb3
   ...
   --  raises Itb_Error with Itb3.Status.Triple_Closed.
   ...
   --  incremental sessions in Itb3.Stream.
   ...
   --  Profile records. A profile record is the JSON object libitb3
   --  accepts in Register, returns from Lookup / Inspect, and embeds
   --  in every blob. Optional keys are omitted when empty / zero.
   --  The binding treats the record as an opaque String; every field
   --  rule is enforced by libitb3.
   ...
   --  duplicate name raises Itb_Error with Itb3.Status.Profile_Exists.
   ...
   --  raises Itb_Error with Itb3.Status.Unknown_Profile.
   ...
   --  The raw libitb3 handle — internal, used by Itb3.Stream to open
