...
package Itb3.Error is

   --  Raised by every Itb3.* subprogram on a non-OK libitb3 status.
   Itb_Error : exception;

   --  Numeric status code encoded in the occurrence's payload.
   --  Returns Itb3.Status.Internal_Error when the payload is not in
   ...
   procedure Raise_For (Status : Itb3.Status.Code)
   with No_Return;
