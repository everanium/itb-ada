...
package body Itb3.Stream is
   ...
   procedure Do_Begin
     (S : in out Session'Class; P : Itb3.Pipeline.Pipeline; Encrypt : Boolean)
   is
   ...
   begin
      ...
      if Encrypt then
         Check (ITB_Triple_EncryptStreamBegin (Itb3.Pipeline.Raw (P), H'Access));
      else
         Check (ITB_Triple_DecryptStreamBegin (Itb3.Pipeline.Raw (P), H'Access));
      end if;
      S.H := H;
   end Do_Begin;
   ...
   procedure Begin_Encrypt
     (S : in out Encrypt_Stream; P : Itb3.Pipeline.Pipeline) is
   begin
   ...
   procedure Begin_Decrypt
     (S : in out Decrypt_Stream; P : Itb3.Pipeline.Pipeline) is
   begin
